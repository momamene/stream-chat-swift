...
class ChannelReadUpdaterMiddleware_Tests: XCTestCase {
    ...
    func test_messageReadEvent_handledCorrectly() throws {
        ...
        let handledEvent = try await { middleware.handle(event: messageReadEvent, session: database.viewContext, completion: $0) }
    ...
    func test_notificationMarkReadEvent_handledCorrectly() throws {
        ...
        // Let the middleware handle the event
        let handledEvent = try await {
            middleware.handle(event: notificationMarkReadEvent, session: database.viewContext, completion: $0)
        }
    ...
    func test_notificationMarkAllReadEvent_handledCorrectly() throws {
        ...
        // Let the middleware handle the event
        let handledEvent = try await {
            middleware.handle(event: notificationMarkAllReadEvent, session: database.viewContext, completion: $0)
        }
    ...
    func test_unhandledEvents_areForwarded() throws {
        ...
        // Let the middleware handle the event
        let handledEvent = try await { middleware.handle(event: startTypingEvent, session: database.viewContext, completion: $0) }
